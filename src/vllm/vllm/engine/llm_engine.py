...
class LLMEngine:
    ...
    def _build_logits_processors(
        ...
        # [Instance] Forced-decode logits processor.
        # When extra_args contains ``output_tokens``, attach a processor that
        # forces the model to generate those exact tokens (used for replay
        # in agentic workflows).  Also supports ``eos_token_id`` override;
        # falls back to the model's EOS token if not provided.
        if (sampling_params.extra_args is not None
                and "output_tokens" in sampling_params.extra_args):
            output_tokens = sampling_params.extra_args["output_tokens"]
